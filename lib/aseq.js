...
aseq.createErrorHandler = function(handle)
{
	...
	return function()
	{
		...
		var callbackOk =
			seqCallback != null && typeof(seqCallback) === 'function';
		...
		if (arguments.length > 1)
		{
			...
			handle(arguments[0], function()
			{
				...
				if (generatorArguments.length > 2)
				{
					...
					if (callbackOk) seqCallback.apply(
						this,
				...
				else
				{
					...
					if (callbackOk) seqCallback(argumentsToArray(arguments));
				}
			});
		}
		else
		{
			...
			// => Error handled
			// => Nothing to do
			if (callbackOk) process.nextTick(seqCallback);
		}
	};
};
